...
class App(tk.Tk):
    ...
    def _build_sidebar(self):
        ...
        tk.Label(logo_frame, text="◈", font=(FONT_MONO, 22),
                 fg=ACCENT, bg=BG_PANEL).pack()
        tk.Label(logo_frame, text="rekordbox-tools",
                 font=(FONT_MONO, 11, "bold"), fg=TEXT, bg=BG_PANEL).pack()
        tk.Label(logo_frame, text="for Rekordbox 7",
                 font=(FONT_MONO, 9), fg=TEXT_DIM, bg=BG_PANEL).pack(pady=(2, 0))
        ...
        for page_id, label in nav_items:
            btn = tk.Label(
                ...
                font=(FONT_MONO, 10), fg=TEXT_DIM, bg=BG_PANEL,
        ...
        # Version footer
        tk.Label(self.sidebar, text="v1.0.0", font=(FONT_MONO, 8),
                 fg=TEXT_DIM, bg=BG_PANEL).pack(side="bottom", pady=12)
    ...
    def _heading(self, parent, title, subtitle=""):
        tk.Label(parent, text=title, font=(FONT_MONO, 18, "bold"),
                 fg=TEXT, bg=BG, anchor="w").pack(fill="x")
        if subtitle:
            tk.Label(parent, text=subtitle, font=(FONT_MONO, 10),
                     fg=TEXT_DIM, bg=BG, anchor="w").pack(fill="x", pady=(2, 0))
    ...
    def _path_row(self, parent, label, var, hint="", browse_dir=True):
        ...
        tk.Label(row, text=label, font=(FONT_MONO, 9, "bold"),
                 fg=TEXT_DIM, bg=BG, width=22, anchor="w").pack(side="left")
        entry = tk.Entry(row, textvariable=var, font=(FONT_MONO, 10),
        ...
        if hint:
            entry.insert(0, hint)
        btn = tk.Label(row, text="Browse", font=(FONT_MONO, 9),
                       fg=ACCENT, bg=BG_INPUT, padx=10, pady=6, cursor="hand2",
                       relief="flat")
        btn.pack(side="left")
        if browse_dir:
    ...
    def _dry_run_button(self, parent, label, command):
        btn = tk.Label(parent, text=label, font=(FONT_MONO, 10),
                       fg=ACCENT, bg=BG_INPUT, padx=18, pady=8, cursor="hand2",
                       relief="flat")
    ...
    def _output_console(self, parent):
        frame = tk.Frame(parent, bg=BG_PANEL, relief="flat",
                         highlightthickness=1, highlightbackground=BORDER)
        frame.pack(fill="both", expand=True, pady=(12, 0))
        txt = tk.Text(frame, font=(FONT_MONO, 9), bg=BG_PANEL, fg=TEXT,
                      insertbackground=ACCENT, relief="flat", bd=0,
                      wrap="word", padx=12, pady=10, state="disabled")
        txt.pack(fill="both", expand=True, side="left")
        sb = tk.Scrollbar(frame, command=txt.yview, bg=BG_PANEL,
                          troughcolor=BG_PANEL, activebackground=BORDER)
    ...
    def _run_script(self, console, script_name, args):
        ...
        if not script_path.exists():
            self._append_console(console,
                f"ERROR: script not found: {script_path}", "red")
            return
            ...
            try:
                while True:
                    ...
                    if "[dry-run]" in line.lower():
                        ...
                        tag = "red"
                    elif line.startswith("===") or line.startswith("---") or line.startswith("SUMMARY"):
                        tag = "accent"
                    elif "->" in line:
                        tag = "green"
                    else:
    ...
    def _build_page_setup(self):
        ...
        self._heading(page, "Setup & Paths",
        ...
        for var, key in [
            ...
            var.trace_add("write", lambda *a, k=key, v=var:
                          self._on_path_change(k, v))

        section = tk.Frame(page, bg=BG_PANEL, relief="flat",
                           highlightthickness=1, highlightbackground=BORDER,
                           padx=20, pady=20)
        section.pack(fill="x", pady=(0, 16))

        tk.Label(section, text="LIBRARY ROOTS", font=(FONT_MONO, 9, "bold"),
                 fg=ACCENT, bg=BG_PANEL).pack(anchor="w", pady=(0, 10))

        self._path_row(section, "Music Root",
                       self._v_music_root,
                       hint="Root folder containing all your DJ music")
        self._path_row(section, "FLAC / Target Root",
                       self._v_flac_root,
                       hint="High-quality folder (relocate target)")
        self._path_row(section, "MP3 / Source Root",
                       self._v_mp3_root,
                       hint="Folder to migrate away from (optional)")
        self._path_row(section, "DELETE folder",
                       self._v_delete_dir,
                       hint="Where cleanup moves unreferenced files")

        save_btn = self._run_button(section, "  Save Paths  ", self._save_paths)

        # Info box
        info = tk.Frame(page, bg=BG_PANEL, relief="flat",
                        highlightthickness=1, highlightbackground=BORDER,
                        padx=20, pady=20)
        info.pack(fill="x")
        tk.Label(info, text="ℹ  WHERE TO FIND REKORDBOX FILES",
                 font=(FONT_MONO, 9, "bold"), fg=ACCENT, bg=BG_PANEL).pack(anchor="w")
        ...
        for label, path in hints:
            ...
            tk.Label(row, text=label, font=(FONT_MONO, 9, "bold"),
                     fg=TEXT_DIM, bg=BG_PANEL, width=30, anchor="w").pack(side="left")
            tk.Label(row, text=path, font=(FONT_MONO, 9),
                     fg=TEXT, bg=BG_PANEL, anchor="w").pack(side="left")
    ...
    def _build_page_relocate(self):
        ...
        self._heading(page, "Relocate Tracks",
                      "Re-point broken or MP3 track paths to a FLAC (or other) target root.")

        opts_frame = tk.Frame(page, bg=BG_PANEL, relief="flat",
                              highlightthickness=1, highlightbackground=BORDER,
                              padx=20, pady=20)
        ...
        row.pack(fill="x", pady=5)
        tk.Label(row, text="Prefer extension", font=(FONT_MONO, 9, "bold"),
                 fg=TEXT_DIM, bg=BG_PANEL, width=22, anchor="w").pack(side="left")
        ext_entry = tk.Entry(row, textvariable=self._v_rel_pref_ext,
                             font=(FONT_MONO, 10), bg=BG_INPUT, fg=TEXT,
                             insertbackground=ACCENT, relief="flat", bd=0,
                             highlightthickness=1, highlightbackground=BORDER,
                             highlightcolor=ACCENT, width=10)
        ext_entry.pack(side="left", ipady=6)

        self._checkbox(opts_frame, "Missing only  (skip tracks that already resolve)", self._v_rel_missing)
        self._checkbox(opts_frame, "All tracks  (re-check even existing paths)", self._v_rel_all)

        console = self._output_console(page)

        self._dry_run_button(page, "  ▷  Dry Run (preview only)  ",
            lambda: self._run_relocate(console, dry=True))
        self._run_button(page, "  ▶  Run Relocate  ",
            lambda: self._run_relocate(console, dry=False))
    ...
    def _run_relocate(self, console, dry):
        ...
        if not target:
            messagebox.showwarning("Missing Path", "Set a Target Root first (Setup tab or field above).")
            return
    ...
    def _build_page_cleanup(self):
        ...
        self._heading(page, "Library Cleanup",
        ...
        self._dry_run_button(page, "  ▷  Dry Run (preview only)  ",
            lambda: self._run_cleanup(console, dry=True))
        self._run_button(page, "  ▶  Run Cleanup  ",
            lambda: self._run_cleanup(console, dry=False))
    ...
    def _build_page_remove_missing(self):
        ...
        self._heading(page, "Remove Missing Tracks",
        ...
        tk.Label(info, text="This performs a soft delete (rb_local_deleted=1) — identical to how "
                 "Rekordbox removes tracks itself. No rows are destroyed. A timestamped backup "
                 "is created automatically before any changes.",
                 font=(FONT_MONO, 9), fg=TEXT_DIM, bg=BG_PANEL,
        ...
        self._run_button(page, "  ▶  Run Remove Missing  ",
            lambda: self._confirm_run_remove_missing(console))
    ...
    def _build_page_strip_comments(self):
        ...
        self._heading(page, "Strip URL Comments",
        ...
        self._dry_run_button(page, "  ▷  Dry Run (preview only)  ",
            lambda: self._run_strip(console, dry=True))
        self._run_button(page, "  ▶  Strip URL Comments  ",
            lambda: self._run_strip(console, dry=False))
