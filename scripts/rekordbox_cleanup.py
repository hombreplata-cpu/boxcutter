...
def run(args):
    ...
    if not scan_root.is_dir():
        raise SystemExit(f"--scan-root is not a valid directory: {scan_root}")
    ...
    if args.delete_dir:
        ...
        delete_dir = (
            onedrive_desktop
            if (Path.home() / "OneDrive" / "Desktop").is_dir()
    ...
    print(f"[scan] Found {len(unreferenced):,} unreferenced files.")
    ...
    print("SUMMARY")
    print("=" * 60)
    print(f"  Audio files scanned          : {total_scanned:,}")
    print(f"  Active in Rekordbox          : {total_scanned - len(unreferenced):,}")
...
def main():
    ...
    parser.add_argument(
        "--scan-root", metavar="DIR", required=True, help="Folder to scan for unreferenced files"
    )
    parser.add_argument(
        "--exclude",
        metavar="DIR",
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Show what would be moved without moving anything"
    )
    parser.add_argument(
        "--extensions", metavar="LIST", help="Comma-separated audio extensions to check"
    )
    parser.add_argument(
        "--db-path",
        metavar="PATH",
