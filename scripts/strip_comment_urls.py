...
def crawl(directories, write):
    ...
    print(f"Files modified: {total_changed}  ({'written' if write else 'dry run -- no changes written'})")
...
def main():
    parser = argparse.ArgumentParser(
        description="Remove URLs from MP3/FLAC comment tags."
    )
    parser.add_argument(
        'directories',
        nargs='+',
        help='One or more directories to crawl recursively.'
    )
    parser.add_argument(
        '--write',
        action='store_true',
