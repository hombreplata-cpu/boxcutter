...
def main():
    ...
    print("[db] {:,} tracks loaded".format(len(contents)))
    ...
    for r in contents:
        ...
        if not os.path.isfile(win_path):
            if args.verbose:
                print("  [MISSING] {} — file not on disk".format(r.Title))
            stats["missing"] += 1
            missing_log.append({
                "id":    r.ID,
                "title": r.Title or "",
        ...
        if db_type != actual_type:
            changes.append("FileType {} → {}".format(db_type, actual_type))
        if db_size != actual_size:
            changes.append("FileSize {:,} → {:,}".format(db_size, actual_size))
    ...
    print("SUMMARY")
    print("=" * 50)
    print("  Total tracks      : {:,}".format(total))
    print("  Fixed{}           : {:,}".format(" (dry-run)" if args.dry_run else "           ", stats["fixed"]))
